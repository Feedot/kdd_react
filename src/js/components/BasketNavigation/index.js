  ...
  render() {
    let basketClassName =
      this.props.quantity !== 0
        ? this.props.classes.className + " active"
        : this.props.classes.className;
    return (
      <div className={basketClassName}>
        <h3>{this.props.name}</h3>
        <div className={this.props.classes.classSum}>
          <span>{this.props.quantity}</span> товар(ов) на сумму:{" "}
        </div>
        <p className={this.props.classes.classNameWith}>
          {this.props.price + "₽"}
        </p>
        <p className={this.props.classes.classNameEmpty}>Пусто</p>
        <button
          onClick={this.showModalWindow.bind(this)}
          className={this.props.classes.classNameButton}
        >
          Оформить
        </button>
      </div>
