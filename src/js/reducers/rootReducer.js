...
export default function reducer ( state = initialState ,action ){
    ...
    if(action.type === 'SHOW_CLOSE_MOBILE_FILTER'){
        ...
        if (state.sidebar.classes.classSideBar === 'side_bar') state.sidebar.classes.classSideBar  = "side_bar active";
    ...
    if(action.type === 'CLOSE_MOBILE_FILTER'){
        ...
        if (state.sidebar.classes.classSideBar === 'side_bar active') state.sidebar.classes.classSideBar  = "side_bar";
    ...
    if (action.type === "CHANGE_MENU_BUTTON_CLASS") {
        ...
        if(action.payload.className === '') {
            ...
            state.navigation.classes.headerClassName = state.navigation.classes.headerClassName + ' active'
    ...
    if(action.type === 'RETURN_CLASSES'){
        ...
        state.navigation.classes.headerClassName = header.substr(0,header.length - 7);
    ...
    if (action.type === "SHOW_TYPE_LIST_ASIDE") {
        ...
        if(state.sidebar.categories.activeItem.className === "") state.sidebar.categories.activeItem.className = 'show'
    ...
    if (action.type === "ADD_INFO_TO_FILTER"){
        ...
        if(action.payload.strim === "from") state.sidebar.filter.from = +action.payload.value
        if(action.payload.strim === "to") state.sidebar.filter.to = +action.payload.value
    ...
    if (action.type === "CHANGE_AMMOUNT"){
        ...
        let newObj = state.productList.items.filter((item)=>{
            ...
            if(item.name === action.payload.name) item.ammount = +action.payload.ammount;
    ...
    if(action.type === 'ADD_TO_BASKET'){
        ...
        let result = state.productList.items.reduce((sum,item)=>{
